...
- [ ] **장중 스캔 유량 초과(1700) — 운영 이슈**(2026-08-04 발견): 장중 스캔의 상세 조회 구간에서 `ka10001`·`ka10081`·`ka20006`이 `1700 허용된 요청 개수 초과(유량=1)`로 간헐 실패. 종목별 실패는 WARNING으로 건너뛰지만, `ka20006`(국면) 실패 시 **해당 주기 스캔 전체가 중단**된다(14:14·14:24 발생, 14:04 스캔은 정상). 원인 추정: 클라이언트 TR별 게이트가 정확히 1.0초 간격이라 키움의 초당 1회 제한 경계를 넘김 — Q-03(유량 실측) 미해결의 결과. 조치 후보: `rate_per_tr_per_sec`를 0.8 등으로 낮춰 여유 확보, `update_regime` 실패 시 스캔을 중단하지 않고 직전 국면으로 진행. **주의**: 주문창 자동 갱신(FR-24)도 `ka10001`을 쓰므로 스캔과 겹치면 경합이 커질 수 있음(발견 시점에는 주문창 미사용 상태였으므로 원인은 아님)
