...
- [ ] **스캔 소요 시간이 주기에 근접(여유 30초)**: 후보가 조금만 늘면 다음 주기와 겹친다. 선택지 — `scan_interval_min` 상향(추천 신선도와 트레이드오프), 위생 필터 강화로 후보 축소, `ka10081` 일봉 재조회 캐싱. 설계 §6이 규정한 "스캔 주기 자동 완화"도 아직 미구현이므로 함께 검토
