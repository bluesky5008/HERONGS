...
- [ ] **스캔 1회 소요가 5분 28초 → 31~55분으로 악화, AC-19 깨짐**(2026-09-22 발견, 원인 미조사): DB의 추천 기록 시각으로 확인 — 2026-08-05에는 09:15·09:25·09:35…로 **정확히 10분 주기**(약 38회/일)였으나, 2026-09-22는 09:41·10:11·10:47·11:21·12:02·12:45·13:17·13:46·14:41로 **29~55분 간격, 9회/일**이다. 스캔이 10분 주기를 매번 넘겨 APScheduler가 겹치는 실행을 건너뛴다. 9/14~9/21도 하루 6~8회로 같은 상태. 추천 갱신 빈도가 1/4로 떨어져 있다
  ...
  - **원인 확정(2026-09-23): 키움 모의투자 서버의 `ka10001`(주식기본정보) 응답 지연.** 코드 문제가 아니다
    - 유량 제한 백오프 가설은 **기각** — 첫 스캔 전체에서 `유량 제한` 로그 **0건**
    - 엔드포인트별 호출 간격 실측(첫 스캔 163종목): `ka10001` **중앙값 9.0초**(평균 9.9초, 최대 30초) / `ka10081` 중앙값 1.0초. 8월 4일 실측은 `ka10001` 60~61회/분 = 콜당 1초 미만이었다
    - DB는 병목이 아니다 — 운영 DB 사본(164MB, daily_price 168만 행)에서 후보당 쿼리 3종 합계 **1ms 미만**(`max(date)` 0.20ms, 20행 0.27ms, 250행 0.34ms)
    - 163종목 × 약 10초 = 약 27분으로 실측 스캔 소요(09:01:06 → 09:27:45, **26분 39초**)와 일치한다
  - **구조적 증폭 요인**: 클라이언트가 완전 순차 호출이라 서버 지연이 그대로 곱해진다. 게이트 상한(TR별 1회/초, 전역 5회/초)에는 여유가 많아 동시 실행이면 지연이 겹쳐 숨는다
  - **조치: DCR-004 승인(2026-09-23) → 구현 완료**([DCR-004](changes/DCR-004-stock-info-daily-cache.md)). ① `stock_info_daily` 캐시 테이블 신설 — `build_candidate`가 유효 기간 안의 행이 있으면 `ka10001`을 호출하지 않는다(FR-26, setting `scan.info_cache_days` 기본 1, `0`이면 캐시 미사용 = 롤백 경로) ② `ingest_daily`가 당일 행을 `merge`로 갱신(FR-27)
    - 함께 해소한 결함: 당일 일봉이 최초 1회만 기록되어 2회차 이후 `ka10081` 응답이 전량 폐기되고 점수 계산의 종가·거래량이 그 종목의 첫 스캔 시점에 고정되던 문제. 실측 증거(2026-09-23 14:40): 09:01 스캔 종목의 당일 거래량이 전일 대비 50.5%, 14:11에 처음 들어온 종목은 109.5%
    - 검증(TDD, 실패→성공): `test_ingest_daily_refreshes_today_row`(당일 종가 70000 고정 → 72000 갱신), `test_stock_info_fetched_once_per_day`(ka10001 2회 → 1회), `test_stock_info_cache_disabled_by_setting`(설정 0이면 2회) — **테스트 84건 전부 통과**. 마이그레이션은 운영 DB 사본으로 확인: `stock_info_daily` 자동 생성, `daily_price` 168만 행·`instrument` 3085행 보존
    - 예상 효과: 후보 400종목 기준 약 59분 → 약 10분. **AC-22(주기 내 완료)는 배포 후 다음 거래일 실측으로 확인해야 한다** — 미달 시 대안 B(상세 조회 동시 실행, 예상 6.7분)를 별도 DCR로 올린다
    - 남은 단점: 당일 첫 스캔은 캐시가 비어 있어 여전히 느리다(163종목 약 27분). 장중 PER·PBR이 당일 첫 조회 시점 가격에 고정된다
...
- [x] ~~**조건검색식 기여도 측정 불가 — 관측성**~~ (2026-08-05 조치): `collect_candidates`의 후보 수집 로그를 `총 N종목 (랭킹 N / 조건검색 <조건명> M건(신규 K), …)` 형태로 확장. 조건식별 반환 건수와 **랭킹에 없던 신규 기여분**을 분리해 남기므로 조건식 갱신 필요 여부를 데이터로 판단할 수 있다. 검증: `test_candidate_log_separates_ranking_and_condition` — 테스트 77건
  ...
  - **판별 결과(2026-09-23 09:01 장중) — 파싱은 정상, 조건식 내용 문제로 확정**: 3종 모두 0건인데 `파싱 0건` 경고가 **0회**였다. 즉 서버 응답의 `data`가 비어 있었던 것이고 필드 키 불일치가 아니다. **조건검색 기능 자체는 복구 완료**이며, 남은 것은 코드가 아니라 **HTS 조건식 3종(HERONGS_LONG/SWING/SCALP)의 내용**이다 — 현재 매칭 0종목이라 후보에 기여할 수 없다. 조건식 재작성은 HTS에서 사용자가 수행해야 한다
  - **HTS 대조 확인(2026-09-23, 사용자)**: HTS에서 조건식 3종을 실행한 결과도 **0행**. 앱의 판별과 일치하므로 **조건검색 경로는 코드 측면에서 종결**이고, 남은 일은 HTS 조건식 내용 재작성뿐이다(앱 변경 불필요)
  - 관측 포인트: 조건식이 다시 종목을 잡기 시작하면 후보가 늘어 스캔 소요가 증가한다
