...
- [x] ~~주문창 보조 정보~~ (2026-08-04 구현 — 별도 작업 `docs/work/20260804-order-quote/`, FR-20~24/AC-13~17): 실사용 피드백(매수 시 현재가·예산 미표시, 매도 시 보유수량 미표시, 호가 없음)에 따라 `GET /api/stocks/{code}/quote` 추가 + 주문창에 현재가·호가 5단·주문가능금액/보유수량 표시, 가격 탭 입력, 3초 자동 갱신. 잔여: 폰 화면 실기 확인, 매도 가능 수량(미체결 매도분 차감) 미해결
- [x] ~~**장중 스캔 유량 초과(1700)로 스캔 주기 전체 중단**~~ (2026-08-04 조치): `ka20006`(국면) 실패 시 해당 주기 스캔이 통째로 중단되던 문제(14:14·14:24 발생) → `intraday_scan`에서 국면 갱신과 스캔의 예외 처리를 분리. 국면 갱신이 실패하면 경고만 남기고 `run_scan`을 그대로 수행한다(`current_regime()`이 DB의 직전 국면을 사용하므로 추천은 계속 가능). 검증: `test_intraday_scan_survives_regime_failure` — 테스트 75건
  - **분당 호출 수 실측(2026-08-04)**: `ka10001` 60~61회/분, `ka10081` 54~61회/분 — 클라이언트 TR별 게이트(1.0초)가 정확히 동작 중이며 키움 한계선에 여유 없이 붙어 있다. 1700은 이 경계에서 간헐 발생하며, 종목별 실패는 이미 WARNING으로 건너뛰고 스캔은 완주한다.
  - **`rate_per_tr_per_sec` 하향(0.8 등)은 기각**: 스캔 1회가 이미 **9분 24초~9분 30초**(14:48:14→14:57:44, 14:58:14→15:07:38)로 10분 주기에 30초밖에 여유가 없다. 25% 느려지면 주기를 초과해 매 주기 겹침·스킵이 발생하므로 역효과.
- [x] ~~**스캔 소요 시간이 주기에 근접(여유 30초)**~~ (2026-08-04 조치 — DCR-003 승인·구현): 랭킹 TR당 조회를 2페이지 → **1페이지**로 축소(setting `scan.ranking_pages`, 기본 1). 실측 후보 523 → 299, 예상 스캔 9.4분 → 5.6분. 유동성 하한(10억)은 유지 — 최근 7일 추천 93종목 중 45%가 30억 미만 구간이라 하한 상향은 추천 실적을 크게 깎는다. 검증: `test_ranking_pages_defaults_to_one_and_is_tunable` — 테스트 76건. **AC-19 통과(2026-08-05 실측)**: 스캔 소요 **9분 24초 → 5분 28초**(04:29:20 시작→04:34:47 종료 등 4회 연속 5분27~29초), 10분 주기 대비 여유 4.5분. 예상치 5.6분과 일치. 후보 426~436종목. 추천 건수는 프로파일당 10건으로 변화 없음(top_n 상한이 이미 10이라 후보 축소의 영향을 받지 않음) → 되돌릴 이유 없음
  - 설계 §6이 규정한 "유량 초과 시 스캔 주기 자동 완화"는 여전히 미구현 — 별도 검토 대상
  - 위생 필터 순서 이슈(참고): 필터는 상세 조회 **전**에 돌고 `avg_trading_value`는 조회 **중**에 갱신되므로, DB 기록이 없는 신규 종목은 첫 스캔에서 필터를 통과한다(하한 10억인데 10억 미만 종목이 추천되는 이유)
- [ ] **스캔 1회 소요가 5분 28초 → 31~55분으로 악화, AC-19 깨짐**(2026-09-22 발견, 원인 미조사): DB의 추천 기록 시각으로 확인 — 2026-08-05에는 09:15·09:25·09:35…로 **정확히 10분 주기**(약 38회/일)였으나, 2026-09-22는 09:41·10:11·10:47·11:21·12:02·12:45·13:17·13:46·14:41로 **29~55분 간격, 9회/일**이다. 스캔이 10분 주기를 매번 넘겨 APScheduler가 겹치는 실행을 건너뛴다. 9/14~9/21도 하루 6~8회로 같은 상태. 추천 갱신 빈도가 1/4로 떨어져 있다
  - 조건검색 수정과 무관하다: 8/5 이후 어느 시점에 발생했고, 9/22 후보 수집은 오히려 30초+ → 13초로 빨라졌다
  - 후보 수는 330~410으로 8월(426~436)과 비슷하므로 후보 수집이 아니라 **종목별 상세 조회 처리량** 쪽을 봐야 한다(유량 제한 백오프 의심)
- [ ] **주의(관측 필요)**: 주문창 자동 갱신(FR-24)도 `ka10001`을 쓰므로 스캔과 겹치면 경합이 커질 수 있다. 2026-08-04 발견 시점에는 주문창 미사용 상태였으므로 1700의 원인은 아니었음
- [ ] **단타(scalp) 신호가 구조적으로 발생 불가 — 결함**(2026-08-05 분석으로 확정, 미수정): 운영 기간 내내 scalp 추천 0건·`scalp_signal` 알림 0건. 원인은 조건검색식이 아니라 **스코어링 입력 누락**이다.
  - `ScalpProfile`은 `rt_momentum`(50) + `strength`(30) + `risk`(20) = 100점 구성인데, `risk`는 `trading_value`, `rt_momentum`의 절반은 `rt_volume_ratio`에서 나온다.
  - **실시간 경로**: `ScalpSignalHandler._evaluate`가 `rt_volume_ratio`와 `trading_value`를 한 번도 채우지 않는다(0B에서 현재가·등락률·체결강도만, 0D에서 호가불균형만 수집). 이상적인 값을 넣어도 **최대 55점** — 판정 기준 `min_score=70`을 넘을 수 없다.
  - **배치 경로**: `build_candidate`도 실시간 3필드를 채우지 않아 **최대 45점** — 추천 기준 60점 미달. scalp가 배치 추천에 한 번도 안 나온 이유.
  - 실측: `PROFILES()['scalp'].score()` — 실시간 최대 55.0 / 배치 최대 45.0 / 전 필드 충족 시 100.0
  - **테스트가 놓친 이유**: `test_scalp_handler_subscribe_evaluate_release`가 `min_score=50.0`으로 낮춰 통과시키고 있어 운영 기본값(70)에서의 도달 불가를 드러내지 못했다.
  - 조치 방향(설계 검토 필요): 0B 실시간 필드에서 누적거래대금·거래량 배율에 해당하는 값을 수집해 채우거나, scalp 가중치·기준점수를 실제 수집 가능한 지표에 맞게 재정의. **장중에 0B 페이로드를 관측해 사용 가능한 필드 번호를 먼저 확정할 것**(현재 사용 중: 10 현재가, 12 등락률, 228 체결강도 / 0D: 121·125 호가총잔량). 테스트도 운영 기본값으로 검증하도록 수정
  - 참고: `kiwoom/client.py` 주석이 가리키는 `doc/kiwoom-rest-api-spec.json`은 **이 기기에 없다**(저장소·홈 디렉터리 전체 검색 확인, 2026-08-05). 필드 확인은 문서가 아니라 실응답 관측으로 해야 한다 — `ka10004` 호가 필드를 확정했던 방식과 동일
- [x] ~~**조건검색식 기여도 측정 불가 — 관측성**~~ (2026-08-05 조치): `collect_candidates`의 후보 수집 로그를 `총 N종목 (랭킹 N / 조건검색 <조건명> M건(신규 K), …)` 형태로 확장. 조건식별 반환 건수와 **랭킹에 없던 신규 기여분**을 분리해 남기므로 조건식 갱신 필요 여부를 데이터로 판단할 수 있다. 검증: `test_candidate_log_separates_ranking_and_condition` — 테스트 77건
  ...
  - **실서버 검증(2026-09-22 장중) — 프로토콜 결함 해소 확정**: 스캔 10회 × 조건식 3종 = **30회 실행 전부 정상 응답, `실패` 0건**(수정 전 100% 타임아웃). 후보 수집 소요 **30초+ → 13초**(09:09:49 스캔 시작 → 09:10:02 수집 완료). 다만 **30회 모두 0건** — 조건식이 실제로 0종목인지 응답 파싱이 어긋난 것인지 미결. 장외에는 조건검색이 원래 0건이라 관측으로 구분할 수 없다
  - **판별용 관측 추가(2026-09-23)**: `run_condition`이 `응답 N행이나 종목코드 파싱 0건 — 샘플 {row}`를 **그 경우에만** WARNING으로 남긴다. 조건식이 진짜 0종목이면 로그가 늘지 않는다. 검증: `test_run_condition_warns_when_rows_present_but_unparsed`, `test_run_condition_quiet_when_condition_matches_nothing` — 테스트 81건
