...
- [x] ~~**스캔 소요 시간이 주기에 근접(여유 30초)**~~ (2026-08-04 조치 — DCR-003 승인·구현): 랭킹 TR당 조회를 2페이지 → **1페이지**로 축소(setting `scan.ranking_pages`, 기본 1). 실측 후보 523 → 299, 예상 스캔 9.4분 → 5.6분. 유동성 하한(10억)은 유지 — 최근 7일 추천 93종목 중 45%가 30억 미만 구간이라 하한 상향은 추천 실적을 크게 깎는다. 검증: `test_ranking_pages_defaults_to_one_and_is_tunable` — 테스트 76건. **다음 거래일 확인 필요**: 실제 스캔 소요 시간(AC-19)과 추천 건수 변화. 과도하게 줄면 `scan.ranking_pages`를 2로 되돌리면 된다(재배포 불필요)
...
- [x] ~~**조건검색식 기여도 측정 불가 — 관측성**~~ (2026-08-05 조치): `collect_candidates`의 후보 수집 로그를 `총 N종목 (랭킹 N / 조건검색 <조건명> M건(신규 K), …)` 형태로 확장. 조건식별 반환 건수와 **랭킹에 없던 신규 기여분**을 분리해 남기므로 조건식 갱신 필요 여부를 데이터로 판단할 수 있다. 검증: `test_candidate_log_separates_ranking_and_condition` — 테스트 77건
  ...
  - **다음 거래일 09:00 이후 이 로그로 확인할 것**: 장중에도 조건검색이 실패하는지(→ 연동 문제), 성공하는데 0건인지(→ 조건식이 과도하게 엄격), 신규 기여가 있는지(→ 조건식 유효). 이 결과 없이는 조건식 수정 여부를 판단하지 않는다
