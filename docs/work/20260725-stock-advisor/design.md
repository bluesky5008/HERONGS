...
작은 단일 사용자 프로젝트이므로 ADR을 별도 파일로 분리하지 않고 §8 「주요 결정」에 통합한다. 승인 시 §8의 각 결정이 승인된 것으로 본다. **v1 승인(2026-07-25)으로 ADR-01\~06은 모두 승인 상태다.**
...
2. Collector: 랭킹 TR 5종(**TR당 1페이지** — DCR-003, setting `scan.ranking_pages`) + 전략별 매핑된 조건검색(ka10172) 실행 → 후보 합집합(중복 제거).
...
### 5.6 매수/매도/홀딩 의견 판정 로직 (FR-06/07, ADR-05)

의견은 프로파일별로 다음 4단계를 순서대로 거쳐 결정된다.

**1) 점수 산출** — 부록 B의 지표 가중치로 0\~100점 계산. 모든 기여 지표와 점수를 score_breakdown에 기록한다.

**2) 우선 규칙(오버라이드)** — 점수보다 먼저 평가되며, 발동 시 점수 매핑을 건너뛴다. 리스크 차단이 종목 매력도보다 항상 우선한다는 원칙의 구현이다.

| 조건 | 의견 | 비고 |
|---|---|---|
| 보유 종목이 전략별 손절선 도달 | 매도(손절) | 텔레그램 알림 동반 (FR-11) |
| 보유 종목이 목표가 도달 | 매도(익절) 또는 부분 익절 제안 | |
| 거래정지·관리종목 편입 (FR-12) | 매도/제외 | 미보유면 분석 자체를 거부 |

**3) 점수 → 의견 매핑** — 보유 여부에 따라 다르게 적용한다.
...
**히스테리시스**: 의견 전환은 경계에 3점 여유를 둔다 — 예: "매수" 상태였던 종목은 67점 미만으로 떨어져야 "관망"으로 전환. 경계 근처 점수 진동으로 의견이 매일 뒤집히는 것을 방지한다. 직전 의견은 opinion 테이블에서 조회한다.

**4) 시장 국면 보정 (FR-14)** — 하락 국면이면 스윙·단타의 매수 기준점을 +10 상향(장기는 미적용). 국면 판정값은 의견 근거에 포함한다.

최종 출력(rationale JSON): stance + score_breakdown + 발동한 오버라이드 규칙 + 적용된 국면 보정 + 히스테리시스 적용 여부. FR-05의 "근거 표시"는 이 구조를 그대로 렌더링한다.

**Phase 2 LLM 결합 방식 (예정, 현재 범위 밖)**: LLM은 의견을 직접 결정하지 않는다. 뉴스·공시 정성 분석으로 **±10점 이내의 점수 보정치와 사유 텍스트만 산출**하고, 1)\~4) 파이프라인은 그대로 유지된다(악재 공시 시 매수 의견을 보류하는 veto 옵션 포함). 판정의 재현성과 성과 검증(FR-16) 유효성을 지키기 위한 구조이며, Profile 플러그인 인터페이스(§2 ScoringEngine)에 보정 단계로 삽입한다.
...
| FR-06/07 | RecommendationService (§5.2), 의견 판정 로직 (§5.6) | AC-03 | 종목 입력 E2E + 판정 규칙 단위 테스트 |
...
점수에서 매수/매도/홀딩 의견으로의 판정 절차(오버라이드·매핑·히스테리시스·국면 보정)는 §5.6에 정의한다.
