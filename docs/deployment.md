...
| **타겟 1** | 맥미니 (macOS) | 실운영 24시간 상시 구동 (모의투자 → 실계좌) | **이관 목표** — §6 절차로 이관 |
| 상시 | NAS DS220j | 백업 저장소 전용 (실행 환경 아님) | 대기 (맥미니에 SMB 마운트 필요) |
...
- 백업 실패 시 텔레그램 경고 발송
- 남은 설정: DS220j SMB 공유를 맥미니에 마운트하고 `.env`의 `HERONGS_BACKUP_DIR`(로컬 실행) 또는 `HERONGS_BACKUP_MOUNT`(compose)로 지정 — §6-A 6단계
- 스냅샷 생성·보관 로직은 단위 테스트로 검증됨. **실 NAS 전송 확인(AC-12)만 잔여**. 타겟 0은 `HERONGS_BACKUP_DIR` 미설정으로 설계상 "생략" 동작 중(2026-07-28 확인)

## 6. 타겟 이전 절차

### 6-A. 타겟 0(개발 PC) → 타겟 1(맥미니) — 다음 이관 목표
